...
The following variables contain values as described below:
    ...
    using this payment scheme, which is okay. A summary of the required
    math is found below:

        Monthly interest rate = (Annual interest rate) / 12.0
        Monthly unpaid balance = (Previous balance) - (Minimum fixed
        monthly payment)
...
debth = balance
monthlyPay = round(debth / 12.0,-1)
while debth > 0:
    for i in range(1,13):
       debth -= monthlyPay
       debth += round(debth * monthlyInterest,2)
    if debth > 0:
        debth = balance
        monthlyPay += 10
